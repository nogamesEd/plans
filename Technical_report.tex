...
\begin{algorithmic}
\STATE $\:$ \COMMENT{read image, determine height and width of image}
...
\STATE $\:$ \COMMENT{crop the image to pixels corner\textsubscript{x}, corner\textsubscript{y}, square\textsubscript{height}, square\textsubscript{width}}
\STATE $\:$ \COMMENT{resize board to 244x244 pixels, and antialias to eliminate noise}
\STATE $\:$ \COMMENT{save into "cropped images" folder with tag of the letter and number of the square}
\STATE $\:$ \COMMENT{adjust values for corner\textsubscript{x}, square\textsubscript{height}, and letter}
\ENDFOR
\STATE $corner_x\:=\:0, square_{height}\:=\:0$
\STATE $\:$ \COMMENT {increment corner\textsubscript{y}, square\textsubscript{width}, and i}
\ENDWHILE
\end{algorithmic}
\end{algorithm}
